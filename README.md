...
- **Google Web Search**: Search queries on Google directly in your default browser on <kbd>Enter</kbd>, with smart local app fallback or prefix triggers (`g ` or `? `).
...
- **Native Libadwaita Preferences**: Modern GTK4 settings dialog with an interactive shortcut recorder widget.
...
| <kbd>Enter</kbd> | Launch selected application or search Google |
| <kbd>Shift</kbd> + <kbd>Enter</kbd> | Always search query on Google in browser |
| <kbd>Escape</kbd> | Dismiss search window |
